export const SuccessMessages = {
  MONGO_CONNECTION_SUCCESS: "MongoDB connection successfull",

  SIGNOUT_SUCCESS: "You have been successfully signed out.",
  SIGNIN_SUCCESS: "Welcome back! You have signed in successfully.",
  SIGNUP_SUCCESS: "Registration successful! Welcome aboard!",
  USER_VERIFIED: "Your account has been verified successfully.",
  ACCOUNT_FROZEN: "Your account has been frozen.",
  FOLLOW_USER: "You are now following",
  UNFOLLOW_USER: "Your are no longer following",

  //Post-related
  POST_DELETED: "The post has been deleted successfully.",
  POST_LIKED: "You liked this post!",
  POST_UNLIKED: "You unliked this post.",
  POST_CREATED: "The post has been created successfully.",
};
